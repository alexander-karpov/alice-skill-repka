...
describe('Main dialog', () => {
    ...
    test('Специальная фраза для рыбки', async () => {
        await text('');
        expect(await tts('золотую рыбку')).toMatch(/кликать золотую рыбку.*приплыла к нему рыбка, спросила/);
    ...
    test('Распознаёт несогласованные прил. и сущ.', async () => {
        ...
        expect(await tts('маленький гордый птица')).toMatch(/маленькая гордая птица за маленькую гордую птицу /i);
    });
    ...
    describe('Счетчики', () => {
        test('New Game', async () => {
            expect(await events('')).toContainEqual(expect.objectContaining({ event_type: 'New Game' }));
        });
        ...
        test('Call Character', async () => {
            ...
            expect(await events('котик')).toContainEqual(
                expect.objectContaining({ event_type: 'Call Character', event_properties: { name: 'котик' } })
            );
        });
        ...
        test('Call Thing', async () => {
            ...
            expect(await events('лопату')).toContainEqual(
                expect.objectContaining({ event_type: 'Call Thing', event_properties: { name: 'лопата' } })
            );
        });
    });
    ...
    describe('Запросы пользователей', () => {
        test('жили были дед да баба…', async () => {
            await text('');
            expect(
                await tts('жили были дед да баба была у них курочка ряба снесла курочка яичко не простое а золотое')
            ).toMatch('курочка яичко за дедку');
        });

        test('администратор за администратора…', async () => {
            await text('');
            expect(await tts('администратор за администратора администратора администратора')).toMatch(
                'администратор за дедку'
            );
        });
        ...
        test('снегурочку и деда мороза…', async () => {
            await text('');
            expect(await tts('снегурочку и деда мороза после нирваны')).toMatch('дед мороз за дедку');
        });

        test('уже вытащили давно…', async () => {
            await text('');
            expect(await tts('я уже вытащили давно что вы тут лечите')).toMatch('Это не похоже на персонажа');
        });

        test('медведь позвал…', async () => {
            await text('');
            expect(
                await tts('медведь позвал почему вот нам там пудрить это же по другому нужно так правильно что ли')
        ...
        test('золотую внучку…', async () => {
            await text('');
            expect(await tts('золотую внучку которая н не золотая')).toMatch('золотая внучка за дедку');
        });

        test('сестру русалку…', async () => {
            await text('');
            expect(await tts('русалка позвала свою еще 1 сестру русалку')).toMatch('сестра русалка за дедку');
        });

        test('черную причерную тучу…', async () => {
            await text('');
            expect(await tts('черную причерную тучу при тучу тучу тучу')).toMatch('звал дедка тучу');
        });

        test('ну и что то мы сдаемся…', async () => {
            await text('');
            expect(await tts('ну и что то мы сдаемся уже хватит уже')).toMatch('не похоже на персонажа');
        });

        test('александра валерьевича…', async () => {
            await text('');
            expect(await tts('он позвал александра валерьевича')).toMatch(/александр валерьевич за дедку/);
        });
        ...
        test('жучка не умеет говорить…', async () => {
            await text('');
            expect(await tts('жучка не умеет говорить она не может никого позвать')).toMatch(/ж\+учка за дедку/);
        });
    });
